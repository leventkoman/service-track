...
export class CustomerController {
    static async getAll(req: Request, res: Response) {
        try {
            ...
            const response = customerResult.map(({users, serviceProviderCustomers, ...customer}) => ({
                ...
                phone: users?.phone,
                ...users?.userProfile,
                serviceProviders: serviceProviderCustomers.map(({
                                                                    firstContactDate,
                                                                    serviceProviders
                                                                }) => ({...serviceProviders, firstContactDate}))
            }));
    ...
    static async createCustomer(req: Request, res: Response) {
        try {
            const user = req.user;
            const {customerType, vatNumber, note, email, phone, fullName, address, avatar, description} = req.body;
            await db.transaction(async (trx) => {
                ...
                const [customerResult] = await trx.insert(customers).values({
                    ...
                    note,
                    userId: userResult.id,
                    vatNumber: CustomerType.corporate ? vatNumber : null 
                }).returning();
    ...
    static async updateCustomer(req: Request, res: Response) {
        try {
            const user = req.user!;
            const {customerType, vatNumber, note, email, phone, fullName, address, avatar, description, id} = req.body;
            await db.transaction(async (trx) => {
